...
Quick-commerce apps today treat cart-building as a **manual, repetitive chore**:
...
QCart AI reimagines the quick-commerce cart as a **conversational, context-aware, and deeply personalized** experience — powered by AWS Bedrock foundation models and a multi-layered intelligence stack.

---

## 💡 Solution Overview

QCart AI is a full-stack intelligent cart assistant that understands natural language, customer behavior, and shopping context to deliver:
...
"I have fever"       →  Urgency-tagged health essentials, pre-prioritized
```
...
| Refinement | Delete → re-search → re-add | "Make it cheaper" / "Remove dairy" |
...
| Delivery optimization | Static threshold banner | AI gap-fillers that match your context |
| Reorder prediction | "Buy again" button | Cycle-aware predictions with timing intelligence |
...
│                        FRONTEND (React + Vite)                       │
│  ┌──────────┐  ┌──────────────┐  ┌──────────┐  ┌───────────────┐  │
│  │ Chat UI  │  │ ForYou Feed  │  │Cart Drawer│  │ Trending/City │  │
│  └────┬─────┘  └──────┬───────┘  └─────┬────┘  └───────┬───────┘  │
└───────┼────────────────┼────────────────┼───────────────┼───────────┘
        │                │                │               │
        ▼                ▼                ▼               ▼
...
│  │  (LLM Reasoning) │   │  (Deterministic)│   │  (Delivery Opt)  │  │
│  └────────┬─────────┘   └───────┬─────────┘   └────────┬────────┘  │
│           │                      │                       │           │
│  ┌────────▼──────────────────────▼───────────────────────▼────────┐ │
...
|---|---|---|---|
| 🧠 **Reasoning** | Brain Module | Understands intent, builds/refines carts, classifies context & urgency | AWS Bedrock (Nova Pro / Llama 70B) |
| 🏷️ **Personalization** | Rule Engine | Generates behavioral tags from order history (coffee_lover, party_host, etc.) | Pure deterministic rules |
| 🎯 **Recommendation** | ForYou Engine | Scores products against customer tags, surfaces personalized deals | Hybrid (rules + LLM copy) |
| 📦 **Optimization** | Gap Engine | Identifies free-delivery gap, suggests context-aware fillers | Deterministic |
...
## ⚙️ Engineering Principles & Production Patterns

We built QCart AI with industry-grade engineering patterns that go far beyond a typical hackathon prototype:

### 1. Model Orchestration & Task-Aware Routing
...
**Result:** Heavy reasoning uses powerful models; lightweight tasks use fast/cheap models. This reduces average latency by ~60% and token cost by ~45% compared to routing everything through a single large model.

### 2. Multi-Level Caching Strategy

We employ a **tiered caching architecture** to eliminate redundant LLM calls:

| Cache Layer | Mechanism | What It Caches | TTL |
|---|---|---|---|
| **Response Cache** | SHA-256 keyed (message + cart state) | Full API responses | 10 min |
| **Catalog Prefix Caching** | Bedrock Converse API prompt caching | Static system prompt + catalog | Session-level |
| **Demo Warmup Cache** | Pre-seeded via `warmup.py` | All demo scenarios + refinements | Until restart |
...
### 3. Token Optimization & Prompt Engineering

Every token counts at scale. We minimize token usage through:

- **Compact catalog serialization** — `json.dumps(rows, separators=(",", ":"))` strips all whitespace, reducing catalog payload by ~30%
- **Intent-aware retrieval** — instead of sending the entire catalog (80+ products) to the LLM, `catalog.retrieve()` selects only the top 40 relevant products based on context detection and keyword matching
- **Structured output enforcement** — strict JSON schema in the system prompt eliminates conversational padding in model responses
- **Minimal history injection** — only preferences + frequently bought items are sent, not raw order logs
...
### 5. Separation of Concerns: AI vs. Deterministic Logic
...
| Product validation | Dict lookup | O(1) lookup against catalog — no model call |
...
### 6. Idempotent & Stateless API Design

- Every `POST /api/cart` call is **self-contained** — the frontend sends the full current cart state, so the backend never needs session storage
- Cache keys are computed from `(message + cart_state)` — identical inputs always produce identical outputs
- No server-side session → horizontally scalable with zero shared state

### 7. Hallucination Guard (Validation Pipeline)
...
### 8. Context-Aware Retrieval (RAG-Lite)
...
Not a static banner — a smart nudge:
...
MongoDB-backed trending products per city, resolved against the product catalog in real-time.
...
| **Backend** | FastAPI (Python) | Async-ready, Pydantic validation, auto-docs |
